...
void csp_service_handler(csp_conn_t * conn, csp_packet_t * packet) {
        ...
        csp_packet_t * ps_packet = NULL;
	switch (csp_conn_dport(conn)) {

	case CSP_CMP:
		/* Pass to CMP handler */
		if (csp_cmp_handler(conn, packet) != CSP_ERR_NONE) {
			//csp_buffer_free(packet);  调用者释放
			return;
		}
		break;
	...
	case CSP_PS: {
		...
		while(i < pslen) {
			...
			//if (packet == NULL)
			ps_packet = csp_buffer_get(CSP_RPS_MTU);
			if (ps_packet == NULL)
                        {
                          break;
                        }
                        ps_packet->id = packet->id;
			/* Calculate length, either full MTU or the remainder */
			ps_packet->length = (pslen - i > CSP_RPS_MTU) ? CSP_RPS_MTU : (pslen - i);

			/* Send out the data */
			memcpy(ps_packet->data, &pslist[i], ps_packet->length);
			i += ps_packet->length;
			if (!csp_send(conn, packet, 0))
                        {}
                        else{}
		        csp_buffer_free(ps_packet);
	...
	if (packet != NULL) {
          ...
          {}
	  //  csp_buffer_free(packet);  /*调用者释放释放*/
	}
