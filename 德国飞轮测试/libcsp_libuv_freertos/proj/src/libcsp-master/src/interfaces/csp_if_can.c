...
int csp_can_process_frame(can_frame_t *frame)
{
	csp_can_pbuf_element_t *buf;
	uint8_t offset;
	...
	case CFP_MORE:
		...
		buf->remain--;
                ...
                if(buf->packet->length > CSP_CAN_MTU)  /*长度超出不处理*/
                {
                  csp_log_error("RX  too long");
                  csp_if_can.frame++;
                  csp_can_pbuf_free(buf);
                  break;
                }
		...
		/* Check for overflow */
		if (((buf->rx_count + frame->dlc - offset) > buf->packet->length) || ((buf->rx_count + frame->dlc - offset) > CSP_CAN_MTU)) /*长度超出不处理*/
                {
		...
		if (buf->rx_count != buf->packet->length)  /*如果实际发送数据和length域数据不一样则*/
			break;
...
int csp_can_tx(csp_iface_t *interface, csp_packet_t *packet, uint32_t timeout)
{
	...
	//csp_buffer_free(packet);  /*这里不能free  由分配者free*/
